...
public class BungisngisWalk : StateMachineBehaviour
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if (m_BungisngisAttack.returnPos)
        {
            ...
            {
                animator.GetComponent<BungisngisHealth>().ReturnReset();
                animator.SetTrigger("Idle");
                m_BungisngisAttack.returnPos = false;
            }
        }

        else if (m_Distance <= m_WalkDistance && !m_BungisngisAttack.isPlayerDied && !m_BungisngisAttack.returnPos)
        {
